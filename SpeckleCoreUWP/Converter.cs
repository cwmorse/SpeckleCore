...
namespace SpeckleCore
{
  ...
  public abstract class Converter
  {
    ...
    {
            ///UWP Edit
            ///Not sure how to serialize with UWP, returning null to allow compile
            return "null";
    ...
    private static IEnumerable<MethodInfo> GetExtensionMethods( Assembly assembly, Type extendedType, string methodName )
    {
            var query = from type in assembly.GetTypes()
                            ///UWP Edit
                            ///.IsSealed and IsGeneric not found, using jst IsNested to allow compile
        ...
        if ( !( obj is SpeckleAbstract ) )
        {
          ...
          if ( toNativeMethods.ContainsKey( type ) )
            return toNativeMethods[ obj.GetType().ToString() ].Invoke( obj, new object[ ] { obj } );
                    ...
                    /// UWP Edit
                    /// GetAssemblies() was not found, getting UnitySpeckleConverter a little more directly
                    /// Probably still a better way to do this
                    List<Assembly> myAss = new List<Assembly>();
                    myAss.Add (Type.GetType("ConverterHack,SpeckleUnityConverter").GetTypeInfo().Assembly);
        ...
        else
        {
          ...
          if ( absObj._type == "ref" )
            return null;
                    ...
                    /// UWP Edit
                    /// GetAssemblies not found. Set to null to allow compile
          ...
          object myObject = null;
                    ...
                    /// UWP Edit
                    /// Commented out to allow compile
                    /*
          try
          {
    ...
    private static object ReadValue( object myObject, object root = null )
    {
      ...
      if ( myObject.GetType().GetTypeInfo().IsPrimitive || myObject.GetType() == typeof( string ) )
        return myObject;
    ...
    {
            /// UWP Edit
            /// return SpeckleNull to allow compile
