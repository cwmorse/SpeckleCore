...
//using System.Timers; //UWP Edit - Assembly doesn't exist
using WebSocketSharp;

using System.Threading; //UWP Edit - used for Timer

namespace SpeckleCore
{

    /// UWP Edit
    /// Replaced [Serializable] with [DataContract] throughout file
    /// Also removed : ISerializable from classes
  ...
  public partial class SpeckleApiClient
  {
    ...
    private void SetReadyTimer( )
    {
            ...
            IsReady = new Timer(CallReadyTimer, new AutoResetEvent(true), 200, 200);
        ...
        /// UWP Edit
        /// Callback Event for System.Threading.Timer
        private void CallReadyTimer(Object stateInfo)
        {
            LogEvent("Checking readiness...");
            if (StreamId == null || ClientId == null || WsConnected == false)
            {
                /// UWP Edit
                /// Replace System.Timers.Timer with System.Threading.Timer 
                //IsReady.Start();
            ...
            OnReady?.Invoke(this, new SpeckleEventArgs() { EventName = "client-ready" });
            IsConnected = true;
            IsReady.Change(Timeout.Infinite, Timeout.Infinite);
            LogEvent("Client is ready!");
        }
    ...
    private void SetWsReconnectTimer( )
    {
            ...
            WsReconnecter = new Timer(CallWsReconnectTimer, new AutoResetEvent(true), 1000, 1000);

            //WsReconnecter = new Timer( 1000 ) { AutoReset = false, Enabled = false };
            //WsReconnecter.Elapsed += 

            //      WsReconnecter = new DispatcherTimer();
            //      WsReconnecter.Interval = new TimeSpan(1000);

            //      WsReconnecter.Tick += ( sender, e ) =>
            //{
        ...
        /// UWP Edit
        /// Callback Event for System.Threading.Timer
        private void CallWsReconnectTimer(Object stateInfo)
        {
            if (IsDisposed) return;
            WebsocketClient.Connect();
        }
    ...
    public void SetupWebsocket( )
    {
      ...
      WebsocketClient.OnOpen += ( sender, e ) =>
      {
        WsConnected = true;
          ...
          /// Replace System.Timers.Timer with System.Threading.Timer 
          //WsReconnecter.Stop();
          WsReconnecter.Change(Timeout.Infinite, Timeout.Infinite);
      ...
      WebsocketClient.OnClose += ( sender, e ) =>
      {
        WsConnected = false;
          ...
          /// Replace System.Timers.Timer with System.Threading.Timer 
          //WsReconnecter.Start();
          WsReconnecter.Change(1000,1000);
    ...
    public void LogError( SpeckleException err )
    {
      OnError?.Invoke( this, new SpeckleEventArgs() { EventName = err.StatusCode.ToString(), EventData = err.Message, EventObject = err } );
        ...
        /// UWP Edit
        /// SerializationInfo not found, removed to allow compile
